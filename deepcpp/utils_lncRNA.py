...
def get_model_dir():
    return pkg_resources.resource_filename(__name__, 'models/')
...
def orf_single(seq):
    ...
    if len(startss) != 0:
        ...
        if (startss[max_len_position] - 3) >= 0 and (startss[max_len_position] + 5) < l:
            newseq_6 = seq[(startss[max_len_position] - 3):       (startss[max_len_position])] + seq[(startss[
                                                                                                          max_len_position] + 3):(
                                                                                                             startss[
                                                                                                                 max_len_position] + 6)]
    ...
    for i in range(6):
        ...
        for j in range(len(seq)):
            if len(seq[j]) == 6:
                temp.append(seq[j][i])
        nu5[i, :] = np.array(
            [temp.count('A') / num, temp.count('C') / num, temp.count('G') / num, temp.count('T') / num])
    return nu5
...
def encode(model_dir, RNA_seq, m_type):
    if m_type == 'human':
        ...
        nc_m = pd.read_csv(model_dir + 'humanlnc_6mermean.csv', header=None, delimiter=',')
        c_m = pd.read_csv(model_dir + 'humanmrna_6mermean.csv', header=None, delimiter=',')
        nc_m = np.array(nc_m)
        c_m = np.array(c_m)
    elif m_type == 'vert':
        ...
        nc_m = pd.read_csv(model_dir + 'vertlnc_6mermean.csv', header=None, delimiter=',')
        c_m = pd.read_csv(model_dir + 'vertmrna_6mermean.csv', header=None, delimiter=',')
        nc_m = np.array(nc_m)
        c_m = np.array(c_m)
    elif m_type == 'insect':
        ...
        nc_m = pd.read_csv(model_dir + 'insectlnc_6mermean.csv', header=None, delimiter=',')
        c_m = pd.read_csv(model_dir + 'insectmrna_6mermean.csv', header=None, delimiter=',')
        nc_m = np.array(nc_m)
        c_m = np.array(c_m)
    else:
        raise ValueError('m_type can not be ' + m_type)
...
def run_single_model(fasta, model_dir, m_type, d_type, output):
    if m_type == 'human':
        if d_type == 'normal':
            ...
            raise ValueError("d_type can not be " + d_type)
    elif m_type == 'vert':
        if d_type == 'normal':
        ...
        raise ValueError("m_type can not be " + m_type)
    records = list(SeqIO.parse(fasta, "fasta"))  # Human_orf_test_nc
    RNA_seq = []
    RNA_id = []
    for i in range(len(records)):
        RNA_seq.append(records[i].seq)
        RNA_id.append(records[i].id)
    maxposition = pd.read_csv(model_dir + featureset, header=None, delimiter=',')  # testcpp_fea535 normal566 sorf450
    maxposition = np.array(maxposition)
    maxposition = [int(maxposition[i]) for i in range(len(maxposition))]
    RNA_data0 = encode(model_dir, RNA_seq, m_type)
...
def pipeline(fasta, prefix, species):
    '''
    ...
    :return:
    '''
    if species not in ['human', 'vert', 'insect']:
        raise ValueError('species could not be ' + species)
    model_dir = get_model_dir()
    run_single_model(fasta, model_dir, species, 'normal', prefix + '_normal_predict_results.csv')
    run_single_model(fasta, model_dir, species, 'sorf', prefix + '_sorf_predict_results.csv')
